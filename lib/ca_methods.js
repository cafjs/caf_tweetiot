...
exports.methods = {
    ...
    async __ca_init__() {
        ...
        this.state.fullName = this.__ca_getAppName__() + '#' +
            this.__ca_getName__();
        this.state.aclMapName = this.$.sharing.fullName(AUTHORIZED_MAP);
        this.state.topics = {};
        this.state.tweets = [];
        this.state.myTweets = [];
        this.state.acl = {};

        const me = json_rpc.splitName(this.__ca_getName__())[0];
        const initialValue = {};
        initialValue[me] = true;
        this.$.sharing.addWritableMap('acl', AUTHORIZED_MAP, {initialValue});

        this.$.sharing.addReadOnlyMap(
            'aclAgg', this.state.aclMapName,
    ...
    async __ca_handleTweet__(topic, cmd, fromCA) {
        if (this.state.topics[topic]) {
            ...
            this.state.tweets.push({from, msg: action.msg, time});
            this.state.tweets = this.state.tweets.slice(
                -this.$.props.maxTweets
            );
            notifyApp(this, this.state);
    ...
    async hello(key, tokenStr) {
        ...
        // Delayed configuration to wait for `fromCloud` initialization
        if (!this.state.meta) {
    ...
    async tweet(msg) {
        ...
        if (isBuzz & isNow) {
            ...
            const time = (new Date()).getTime();
            this.state.myTweets.push({msg, time});
    ...
    async buzz(delay, duration) {
        const bundle = this.$.iot.newBundle(0); // No propagation delay
        ...
        notifyIoT(this, `BUZZ: NOW+${delay} for ${duration} milliseconds`);
        return this.getState();
    },
    ...
    async getState() {
        const $$ = this.$.sharing.$;
        this.$.react.coin();
        this.state.acl = $$.acl && $$.acl.toObject() || this.state.acl;
